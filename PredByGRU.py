...
from keras.layers import Dense, LSTM, Activation, Input, Dropout, GRU
...
class CLIENT_STATE:
    ...
    def gru_model(self, x_train, y_train, yy_train, x_test, y_test, yy_test):

        # input model
        inputs_01 = Input(shape=(self.look_back, x_train.shape[2]), name='input_layer')

        gru = GRU(units=50, name='lstm_layer')(inputs_01)
        ...
        np.savetxt('./pred_result/gru_acc_loss6.txt', np_out)
...
if __name__ == "__main__":
    ...
    # pred poi by lstm
    fc.gru_model(x_train, y_train, yy_train, x_test, y_test, yy_test)
