...
from keras.layers import Dense, LSTM, Activation, Input, Dropout, SimpleRNN
...
class CLIENT_STATE:
    ...
    def lstm_model(self, x_train, y_train, yy_train, x_test, y_test, yy_test):
        ...
        #                          tra_poi_acc, 2, tra_sl_acc, val_sl_acc], axis=0)
        #
        # np.savetxt('./pred_model/loss_and_acc_tmp.txt', np_out)
    ...
    def rnn_model(self, x_train, y_train, yy_train, x_test, y_test, yy_test):

        # bulit model
        inputs_01 = Input(shape=(self.look_back, x_train.shape[2]), name='input_layer')

        # lstm = LSTM(units=self.lstm_units, name='lstm_layer')(inputs_01)
        rnn = SimpleRNN(units=32, name='rnn_layer')(inputs_01)
        ...
        np.savetxt('./pred_result/rnn_acc_loss6.txt', np_out)
...
if __name__ == "__main__":
    ...
    fc.rnn_model(x_train, y_train, yy_train, x_test, y_test, yy_test)
